...
- apply the `vitamin2/laravel-sync` package's public API (config, `sync`, `sync:list`, `sync:commands`, `sync:backups-clean`, `sync:test-connection`, `sync:doctor`) in the smallest correct way
...
### 11. Check readiness for a real sync (optional)
...
check as step 10) and `rsync` installed on the remote. Omit `{remote}` to pick one interactively, or pass
